...
CMesh* CMesh::CreateFromContainer(CFBXLoader& _loader)
{
	...
	if (pMesh->IsAnimMesh())
	{
		...
		pMesh->m_pBoneInverse->Create(sizeof(Matrix), (UINT)vecOffset.size(), SB_TYPE::SRV_ONLY, vecOffset.data());
		...
		pMesh->m_pBoneFrameData->Create(sizeof(tFrameTrans), (UINT)vecOffset.size() * iFrameCount
			, SB_TYPE::SRV_ONLY, vecFrameTrans.data());
	}
...
int CMesh::Load(const wstring& _FilePath)
{
	...
	if (m_vecAnimClip.size() > 0 && m_vecBones.size() > 0)
	{
		...
		m_pBoneInverse->Create(sizeof(Matrix), (UINT)vecOffset.size(), SB_TYPE::SRV_ONLY, vecOffset.data());
